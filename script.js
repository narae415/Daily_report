...
function clearText(targetId) {
    if (confirm("내용을 지우시겠습니까?")) {
        ...
        updateCount(targetId, targetId === 'input_box' ? 'input_count' : (targetId === 'jira_input_box' ? 'jira_input_count' : ''));
        saveConfig();
    }
}
...
function copyToClipboard(text, successMsg) {
    if (!text || !text.trim()) return showToast("복사할 내용이 없습니다.", true);
...
function renderHistory() {
    ...
    list.innerHTML = history.length ? '' : '<div style="text-align:center; color:#888; font-size:12px; padding:20px;">기록 없음</div>';
...
function saveJiraToHistory() {
    ...
    if (!text.trim()) return showToast("데이터가 없습니다.", true);
    saveHistory(text);
    showToast("히스토리에 저장되었습니다.");
}
...
function deleteHistory() {
    ...
    if (!cbs.length) {
        if (confirm("전체 삭제하시겠습니까?")) { localStorage.removeItem('dailyReportHistory'); renderHistory(); showToast("삭제 완료"); }
        return;
    }
...
function processText() {
    ...
    if (!input.join('').trim()) return showToast("변환할 내용이 없습니다.", true);
    ...
    input.forEach(line => {
        ...
        if (!clean) { result.push(""); return; }
        if (clean.startsWith('■') || clean === '[업무]') { result.push(clean); return; }
...
function saveConfig() {
    const data = {
        ...
        smart: document.getElementById('smart_input').value,
...
function loadConfig() {
    ...
    if (data) {
        ...
        document.getElementById('smart_input').value = data.smart || "";
...
window.onload = () => {
    ...
    document.querySelectorAll('textarea, input').forEach(el => {
        el.addEventListener('input', () => {
            ...
            if (el.id === 'jira_input_box') updateCount('jira_input_count', 'jira_input_count');
        });
    });
};